...
class EeroClient:
    ...
    def __init__(
        ...
        """
        self._api = EeroAPI(
            session=session, cookie_file=cookie_file, use_keyring=use_keyring
        )
    ...
    def clear_cache(self) -> None:
        ...
        for cache_key in self._cache:
            if (
                isinstance(self._cache[cache_key], dict)
                and "data" in self._cache[cache_key]
            ):
    ...
    async def get_networks(self, refresh_cache: bool = False) -> List[Network]:
        ...
        if not self._api.preferred_network_id and networks_data:
            self._api.set_preferred_network(networks_data[0].get("id"))
    ...
    async def set_guest_network(
        ...
        result = await self._api.networks.set_guest_network(
    ...
    async def set_device_nickname(
        ...
        result = await self._api.devices.set_device_nickname(
            network_id, device_id, nickname
        )
